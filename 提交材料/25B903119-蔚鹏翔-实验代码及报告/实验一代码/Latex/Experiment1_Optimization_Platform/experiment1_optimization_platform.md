# 实验一报告：基于 Python-Julia 混合平台的典型优化算法实现与验证
...
本实验的结论是：不同优化方法的优势与适用场景差异明显。一阶方法简单稳定但在狭长谷地中收敛慢；二阶方法利用曲率信息后迭代效率高；无导数方法适合梯度不可得或不可靠的场景；ADMM 适合将光滑损失和非光滑稀疏正则分裂处理。
...
为了同时满足这三类要求，本实验没有把所有工作都放在单一语言中完成，而是采用 Python-Julia 混合结构。Python 部分实现算法主体和实验分析，Julia/JuMP 部分作为基准求解器。这样的安排有明确分工：

- Python 适合快速实现数值算法、保存迭代轨迹、绘制图像和整理表格；
- JuMP 适合用接近数学表达式的形式建立优化模型，并调用 Ipopt 等成熟求解器；
- 手写算法与 JuMP 基准放在同一测试问题上比较，可以判断实现是否正确，也可以观察不同算法的实际收敛行为。
...
从实验验收和报告讲解角度看，本实验还强调“可展示性”。一方面，`run_all.py` 可以一键重新生成 JuMP 基准、算法轨迹、CSV 表格和图片；另一方面，报告中的每张图都对应明确的算法现象：轨迹图说明算法搜索路径，收敛曲线说明目标函数下降速度，柱状图说明迭代效率，ADMM 图说明变量分裂后的残差收敛和稀疏恢复效果。因此读者仅根据报告和附录代码，就可以复现实验流程并理解每个结果的含义。
...
| 类型 | 路径 | 说明 |
|---|---|---|
| 代码工具 | `tools/Experiment1_Optimization_Platform/` | Python 算法、运行入口、Julia/JuMP 脚本 |
| 分析结果 | `report_assets/Experiment1_Optimization_Platform/` | JSON 原始结果、CSV 表格、PNG 图像 |
| Markdown 报告 | `Latex/Experiment1_Optimization_Platform/experiment1_optimization_platform.md` | 本报告正文 |
| LaTeX/PDF | `Latex/Experiment1_Optimization_Platform/` | LaTeX 源文件和 PDF |
...
其中 `environment.yml` 安装 Python 科学计算包和 Julia。Julia/JuMP 部分的依赖由 `tools/Experiment1_Optimization_Platform/julia/Project.toml` 管理，首次运行时脚本会自动执行 `Pkg.activate` 和 `Pkg.instantiate`，安装 JuMP、Ipopt 和 JSON。由于首次预编译耗时较长，脚本将 JuMP 调用超时时间设置为 600 秒。
...
| 求解器 | 状态 | $x$ | $y$ | 目标函数 |
|---|---|---:|---:|---:|
| JuMP/Ipopt | LOCALLY_SOLVED | 0.9999999999999899 | 0.9999999999999792 | $1.3289\times10^{-28}$ |

该结果与解析最优点一致，因此后续所有算法的距离误差均以 JuMP/Ipopt 解为基准。为了避免某个外部求解器异常导致实验中断，代码仍保留解析解和 SciPy BFGS 作为备用校验，但本次正式结果使用的是 JuMP/Ipopt 基准。
...
图像中有些轨迹看起来“跳得比较远”，这不是因为所有超参数都失控，而是不同算法的更新机制本来不同。固定步长 GD 每步很小；Adam 会因自适应缩放表现得较平滑；Newton 和阻尼 Newton 会根据 Hessian 直接估计曲率校正方向，因此早期步长可能较大；PSO 是群体搜索方法，它记录的是群体最优点随搜索变化的轨迹，也不会像梯度下降那样连续沿局部负梯度移动。为了避免这些轨迹互相遮挡，重新绘图时将一阶、二阶和无导数方法分成三个子图，并使用不同颜色和 marker 表示不同算法，同时将 JuMP/Ipopt 基准点放在最上层。
...
| 方法 | 步长选择 | 优点 | 局限 |
|---|---|---|---|
| 固定步长 GD | 预先指定固定 $\alpha$ | 实现最简单，每步成本低 | 对步长敏感，曲率变化大时容易慢或不稳定 |
| Armijo GD | 每步回溯搜索 $\alpha$ | 能保证充分下降，更稳健 | 每步可能多次计算函数值，本质仍是一阶方法 |
...
如果当前位置附近的二阶模型可靠，Newton 法可以非常快地接近最优点。本实验中 Newton 法只需 6 步即满足收敛判据，最终目标函数为 $3.4326\times10^{-20}$。这说明对于低维、Hessian 可显式计算且条件较好的阶段，二阶信息极其有效。
...
BFGS 不直接计算 Hessian，而是维护逆 Hessian 的近似矩阵。它利用相邻两步的位移和梯度差更新曲率近似，因此属于拟二阶方法。BFGS 的优点是比 Newton 法更适合 Hessian 不方便显式计算的场景。本实验中 BFGS 34 步收敛，最终目标函数为 $2.7456\times10^{-17}$，说明变尺度思想可以在计算成本和收敛速度之间取得较好折中。
...
三次正则化 Newton 法也是非凸优化中重要的二阶扩展方法，它通过在二阶模型中加入三次正则项改善 Hessian 不定时的稳定性。但为了保证本实验代码主线清晰和结果可复现，本实验将 BFGS 作为变尺度法代表，将三次正则化 Newton 留作后续扩展方向。
...
本实验设置 40 个粒子，搜索区域为 $[-3,3]\times[-1,4]$，迭代 300 步。最终 PSO 达到目标函数值 $2.6742\times10^{-13}$，距离 JuMP 基准点 $7.1613\times10^{-7}$。PSO 的轨迹不是单个点沿梯度方向连续移动，而是群体最优解随搜索过程变化，因此在轨迹图中的形态和梯度方法不同。
...
其中第一项是光滑二次损失，第二项是非光滑 L1 正则。L1 正则可以诱导稀疏性，但由于绝对值函数在 0 点不可导，直接用普通梯度下降处理并不自然。ADMM 的做法是引入辅助变量 $z$：

在这个模型中，$A$ 是数据矩阵，$b$ 是观测向量，$x$ 是需要估计的回归系数。第一项 $\frac12\|Ax-b\|_2^2$ 衡量模型预测 $Ax$ 与观测 $b$ 的拟合误差；第二项 $\lambda\|x\|_1$ 控制模型稀疏性。所谓稀疏回归，是指希望最终的系数向量中大部分分量为 0，只保留少量真正重要的特征。本实验生成的合成数据中，真实非零系数只有 8 个，因此可以直接比较 ADMM 是否恢复出主要非零结构。

L1 正则与 L2 正则的区别也体现在这里。L2 正则通常使参数变小，但很少产生严格的零；L1 正则由于绝对值函数在 0 点存在尖角，配合近端或 ADMM 方法时可以把小系数直接截断为 0。因此 Lasso 不只是拟合数据，也具有特征选择作用。
...
实验构造 $m=120,n=50$ 的稀疏线性回归问题，真实非零系数数目为 8，噪声标准差为 0.01，参数设置为 $\lambda=0.08,\rho=1.0$，迭代 300 步。最终结果如下：
...
其中 primal residual 衡量 $x=z$ 约束的违反程度，dual residual 衡量相邻两次 $z$ 更新的变化。二者最终接近机器精度，说明 ADMM 迭代已经达到稳定的一致状态。
...
| 算法 | 是否收敛 | 迭代数 | 最终目标值 | 梯度范数 | 距 JuMP 基准距离 |
|---|---:|---:|---:|---:|---:|
| Gradient Descent | False | 10000 | $5.8528\times10^{-5}$ | $6.8792\times10^{-3}$ | $1.7068\times10^{-2}$ |
| Armijo GD | False | 10000 | $2.7098\times10^{-10}$ | $2.3128\times10^{-5}$ | $3.6827\times10^{-5}$ |
| Adam | True | 3930 | $1.0702\times10^{-12}$ | $9.9777\times10^{-7}$ | $2.3149\times10^{-6}$ |
| Newton | True | 6 | $3.4326\times10^{-20}$ | $8.2857\times10^{-9}$ | $1.8507\times10^{-11}$ |
| Damped Newton | True | 21 | $3.7440\times10^{-21}$ | $4.4733\times10^{-10}$ | $1.3510\times10^{-10}$ |
| BFGS | True | 34 | $2.7456\times10^{-17}$ | $8.8346\times10^{-8}$ | $1.0865\times10^{-8}$ |
| PSO | True | 300 | $2.6742\times10^{-13}$ | - | $7.1613\times10^{-7}$ |

![Rosenbrock 迭代轨迹](../../report_assets/Experiment1_Optimization_Platform/figures/rosenbrock_contours_trajectories.png)
...
![Rosenbrock 收敛曲线](../../report_assets/Experiment1_Optimization_Platform/figures/rosenbrock_convergence.png)

图 2 使用统一横轴绘制 $f(x_k)-f^\*$。对于已经提前达到收敛阈值的方法，曲线在后续横轴上保持最终值，这样可以在同一迭代尺度下比较所有算法。图中可以看到，一阶方法需要大量迭代逐渐降低目标函数；Newton、阻尼 Newton 和 BFGS 在早期迅速下降并进入平台区；PSO 虽然没有梯度信息，也能在有限迭代内逼近最优值。

![Rosenbrock 算法对比柱状图](../../report_assets/Experiment1_Optimization_Platform/figures/rosenbrock_algorithm_comparison.png)
...
![ADMM-Lasso 目标值和残差](../../report_assets/Experiment1_Optimization_Platform/figures/admm_lasso_objective_residuals.png)

图 4 左侧绘制目标函数相对最终最小值的 gap，并使用对数坐标轴。这样可以观察快速下降阶段和后期平台阶段。右侧绘制 primal residual 和 dual residual，二者都使用对数坐标轴。残差前期下降较快，后期接近机器精度后变平，这是数值算法中常见的稳定现象，并不表示算法失效。

![ADMM-Lasso 稀疏解恢复](../../report_assets/Experiment1_Optimization_Platform/figures/admm_lasso_solution_recovery.png)

图 5 对比真实稀疏系数和 ADMM 恢复系数，并加入 $y=0$ 的横向虚线。由于真实信号本身大部分坐标为零，图中大量位置接近零是 Lasso 稀疏模型的预期结果。零线可以帮助观察哪些系数被压到零，哪些位置保留了显著非零值。结果表明，ADMM 基本恢复出主要非零结构，同时将大部分无关系数置零。

## 9. 总结

本实验完成了一个结构清晰、可复现的优化算法测试平台，并围绕 Rosenbrock 函数和 Lasso 问题验证了多类优化方法。实验得到以下结论：

1. JuMP/Ipopt 可以作为可靠的基准求解器，用来校验手写算法在测试函数上的结果。
2. 固定步长 GD 稳定但收敛慢；Armijo 线搜索能显著改善固定步长问题，但在 Rosenbrock 谷地中仍需要大量迭代。
3. Adam 的自适应缩放提升了一阶方法的实用性，在本实验中能达到严格梯度收敛阈值。
4. Newton、阻尼 Newton 和 BFGS 利用曲率信息后迭代效率明显更高，尤其适合低维且目标函数可微的测试问题。
5. PSO 不需要梯度，在低维连续优化中也能找到较高质量近似解，但其轨迹解释应区别于梯度方法。
6. ADMM 通过变量分裂把光滑损失和非光滑 L1 正则分开处理，软阈值步骤能够产生严格稀疏解。
...
5. James Kennedy and Russell Eberhart. Particle Swarm Optimization. IEEE International Conference on Neural Networks, 1995.
6. Miles Lubin, Oscar Dowson, Joaquim Dias Garcia, Joey Huchette, Benoit Legat, and Juan Pablo Vielma. JuMP 1.0: Recent improvements to a modeling language for mathematical optimization. *Mathematical Programming Computation*, 2023.

## 附录：代码与结果目录说明

| 路径 | 说明 |
|---|---|
| `tools/Experiment1_Optimization_Platform/run_all.py` | 一键运行全部实验 |
| `tools/Experiment1_Optimization_Platform/src/first_order.py` | 固定步长 GD、Armijo GD、Adam |
| `tools/Experiment1_Optimization_Platform/src/second_order.py` | Newton、阻尼 Newton、BFGS |
| `tools/Experiment1_Optimization_Platform/src/derivative_free.py` | PSO |
| `tools/Experiment1_Optimization_Platform/src/admm_lasso.py` | ADMM-Lasso |
| `tools/Experiment1_Optimization_Platform/src/jump_baseline.py` | JuMP 基准调用与备用校验逻辑 |
| `tools/Experiment1_Optimization_Platform/julia/jump_rosenbrock.jl` | JuMP/Ipopt Rosenbrock 求解脚本 |
| `report_assets/Experiment1_Optimization_Platform/raw/` | 原始 JSON 结果 |
| `report_assets/Experiment1_Optimization_Platform/tables/` | CSV 汇总表 |
| `report_assets/Experiment1_Optimization_Platform/figures/` | 报告图片 |
| `Latex/Experiment1_Optimization_Platform/` | LaTeX 源文件与 PDF |

## 附录：核心算法代码摘录

本附录只列出报告讲解中最关键的实现片段。完整代码位于 `tools/Experiment1_Optimization_Platform/`；这里保留目标函数、线搜索、主要更新公式和 JuMP 建模代码，便于对照正文解释算法流程。

### Rosenbrock 函数、梯度、Hessian 与 Armijo 线搜索
...
### 二阶与拟二阶算法：Newton、阻尼 Newton 和 BFGS
