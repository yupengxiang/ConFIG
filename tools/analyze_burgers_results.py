...
Examples:
    ...
    python3 tools/analyze_burgers_results.py --run-root PINN_trained/Burgers --methods standard_1000 pcgrad_1000 config_1000
    python3 tools/analyze_burgers_results.py --run-root PINN_trained/Burgers --methods standard_30000_full pcgrad_30000_full config_30000_full mconfig_30000_full
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--run-root", default="PINN_trained/Burgers", help="Experiment root.")
    parser.add_argument("--methods", nargs="+", default=DEFAULT_METHODS, help="Run folder names.")
    parser.add_argument("--out-dir", default="report_assets", help="Output directory for figures/JSON.")
...
def main() -> None:
    ...
    fig, axes = plt.subplots(len(representative_projects), 2, figsize=(8, 2.8 * len(representative_projects)), dpi=160, constrained_layout=True)
    if len(representative_projects) == 1:
        axes = np.expand_dims(axes, 0)
    for row, (label, project_path) in enumerate(representative_projects):
        network = load_network(project_path, args.device)
        _mse, _mse_grid, prediction = run_test(network, simulation_data, device=args.device)
        pred_im = axes[row, 0].imshow(prediction, aspect="auto", origin="lower", extent=[-1, 1, 0, 1], cmap="RdBu_r")
        axes[row, 0].set_title(f"{label}: prediction")
        axes[row, 0].set_xlabel("x")
        axes[row, 0].set_ylabel("t")
        fig.colorbar(pred_im, ax=axes[row, 0], fraction=0.046, pad=0.04)

        error = np.abs(simulation_data - prediction)
        err_im = axes[row, 1].imshow(error, aspect="auto", origin="lower", extent=[-1, 1, 0, 1], cmap="magma")
        axes[row, 1].set_title(f"{label}: absolute error")
        axes[row, 1].set_xlabel("x")
        axes[row, 1].set_ylabel("t")
        fig.colorbar(err_im, ax=axes[row, 1], fraction=0.046, pad=0.04)
